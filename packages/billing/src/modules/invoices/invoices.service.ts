...
export class InvoicesService extends BaseService<T> {
	...
	async selectInvoicesForPeriod(date_from: string, date_to: string, grannularity: string, account_id: number): Promise<T[]> {
		const sql = `SELECT amount_paid, paid_at
                   FROM billing_invoices
                   WHERE amount_paid == amount_total AND
				   account_id == ${account_id}
				   paid_at IS NOT NULL AND
				   paid_at > ${date_from} AND
				   paid_at <= ${date_to}
				   `
		const invoices = await this.query.raw(this.repository, sql)

		/* TODO: aggregation logic
		with grannularity = DAY / WEEK / MONTH
		 */
	...
	async downloadInvoice(user, invoice_id): Promise<T> {
		const file = await this.storageService.read(
			`invoices/${user.user_id}/${invoice_id}`,
