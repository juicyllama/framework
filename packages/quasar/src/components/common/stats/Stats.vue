<template>
    ...
    <template v-else>
        ...
        <div v-else class="JLStats__section">
            ...
            <Vue3autocounter v-if="isAnimation && value.signOnTheLeft" :startAmount="0" :endAmount="Number(value.value)" :duration="DURATION_SECONDS" :prefix="value.valueSign" decimalSeparator='.' :decimals='2' :autoinit='true'/>
            <Vue3autocounter v-if="isAnimation && !value.signOnTheLeft" :startAmount="0" :endAmount="Number(value.value)" :duration="DURATION_SECONDS" :suffix="value.valueSign" decimalSeparator='.' :decimals='2' :autoinit='true'/>
...
const generateValueLine = (item) => {
    if (!item) return
    if(item.signOnTheLeft)
        return `${item.valueMeasurement} ${item.valueSign}${item.value}`
