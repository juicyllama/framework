...
import { CompetitorsInOrganicSearchParams, DomainOrganicSearchKeywordsParams, DomainVsDomainParams } from '../../utils/intefaces'
...
export class DomainReportService {
	...
	async competitorsInOrganicSearch(queryParams: Partial<CompetitorsInOrganicSearchParams>) {
		...
		try {
			const response = await this.api.get(domain, `${ENDPOINT}/?key=${this.key}&type=domain_organic_organic&${qs}`)
